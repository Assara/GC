...
// Canonicalizer specialized to aggregate_transient_graph.  Vertex zero is fixed;
...
// general GraphStandardizer: the aggregate transient representation has different
...
>
class aggregate_transient_standardizer {
	public:
		using graph_type = aggregate_transient_graph<N_VERTICES, N_EDGES, FieldType>;
		...
		static_assert(N_NONROOT <= 64,
			"aggregate transient refinement supports at most 64 nonroot vertices");
		...
		canonicalization_result standardize_with_info(
			...
			if constexpr (N_NONROOT == 0) {
				...
				// both resolves rare color collisions and gives the exact root-fixing
		...
		struct canon_builder {
			...
			void initialize_colors(
				...
				for (std::size_t local = 0; local < N_NONROOT; ++local) {
					...
					const bool has_root_support = graph.has_root_support(vertex);
					const hash_type support_degree = static_cast<hash_type>(
						std::popcount(adjacency[local]) + has_root_support
					);
					// Vertex zero is not itself refined.  Preserve its incidence as
					// an explicit bit alongside the full degree in the simple support
					// graph.  The packed pair is injective before hashing.
					colors[local] = hash(
						(support_degree << 1)
						| static_cast<hash_type>(has_root_support)
					);
				}
