...
// Canonicalizer for the simple rooted support graph.  Vertex zero remains
// fixed, so only vertices 1,...,N_VERTICES-1 are refined, individualized, and
// permuted.  Edge counts and allocation data are deliberately absent from the
// certificate: the canonical key is exactly the rooted simple support graph.
template <
	Int N_VERTICES,
	Int N_EDGES,
	typename FieldType = fieldType
>
class support_transient_standardizer {
	public:
		using graph_type = support_transient_graph<
		...
		static_assert(N_NONROOT <= 64,
			"support transient refinement supports at most 64 nonroot vertices");
		using nonroot_permutation = typename graph_type::nonroot_permutation;
		using core_adjacency_rows = std::array<std::uint64_t, N_NONROOT>;

		struct canonicalization_result {
			graph_type canonical_graph;
			// Order of the automorphism group which fixes vertex zero.
		...
		canonicalization_result standardize_with_info(
			...
			if constexpr (N_NONROOT == 0) {
				...
				const core_adjacency_rows adjacency
					= create_adjacency_rows(input);
				auto [attempts, valid_attempts]
					= create_final_attempts(input, adjacency);

				graph_type best = input.permuted_nonroot(
					attempts[valid_attempts.front()].create_nonroot_permutation()
				);
				std::uint64_t automorphism_count = 1;

				// Refinement colors are only invariants.  Materializing every final
				// survivor and comparing the exact support certificates both resolves
				// possible color collisions and counts all rooted automorphisms.
				for (std::size_t i = 1; i < valid_attempts.size(); ++i) {
					graph_type candidate = input.permuted_nonroot(
	...
	private:
		...
		struct canon_builder {
			...
			void initialize_colors(
				...
				for (std::size_t local = 0; local < N_NONROOT; ++local) {
					...
					const bool root_adjacent = graph.has_root_support(vertex);
					const hash_type support_degree = static_cast<hash_type>(
						std::popcount(adjacency[local]) + root_adjacent
					);
					// The full support degree alone does not identify incidence with
					// the fixed root.  Pack that incidence bit separately before
					// hashing the initial vertex invariant.
					colors[local] = hash(
						(support_degree << 1)
						| static_cast<hash_type>(root_adjacent)
					);
				}
				...
				for (std::size_t group = 0;
					...
					const std::size_t other_end
						...
						? static_cast<std::size_t>(
							other.group_separators[group]
						)
						: N_NONROOT;

					if (begin < end && other_begin < other_end) {
						const hash_type color
							= colors[vertex_groups[begin]];
		...
		static core_adjacency_rows create_adjacency_rows(
			...
			for (std::size_t first = 0; first < N_NONROOT; ++first) {
				for (std::size_t second = first + 1;
					 second < N_NONROOT;
					 ++second) {
					if (graph.has_core_support_edge(
							static_cast<vertex_type>(first + 1),
							static_cast<vertex_type>(second + 1)
						)) {
