...
// Unrestricted Gra-style insertion in the GC_2 setting: insert `inner` into
// `outer_vertex` of `outer`, and sum over all functions from the incident
// half-edges to the vertex set of `inner`. This drops the low-valence
// restriction on the composed graph.
template <typename OuterGraph, typename InnerGraph>
VectorSpace::LinComb<
	GraInsertLowValentGraph<OuterGraph, InnerGraph>,
	typename OuterGraph::Field
> gra_insert_vertex_unrestricted_raw(
	const OuterGraph& outer,
	Int outer_vertex,
	const InnerGraph& inner,
	typename OuterGraph::Field coefficient = typename OuterGraph::Field{1}
	...
	static_assert(OuterGraph::N_HAIR == 0 && InnerGraph::N_HAIR == 0,
		"gra_insert_vertex_unrestricted_raw currently only supports hairless graphs");
	static_assert(OuterGraph::C_ == 0 && OuterGraph::D_ == 1,
		"gra_insert_vertex_unrestricted_raw is specialised to GC_2 conventions");
	static_assert(InnerGraph::C_ == 0 && InnerGraph::D_ == 1,
		"gra_insert_vertex_unrestricted_raw is specialised to GC_2 conventions");
	static_assert(std::is_same_v<typename OuterGraph::Field, typename InnerGraph::Field>,
		"outer and inner graph must use the same coefficient field");
	...
	auto incident_half_edges = outer.adjacent(outer_vertex);
	if (incident_half_edges.empty()) {
		return result;
	}
	std::sort(incident_half_edges.begin(), incident_half_edges.end());

	bigInt reserve_count = 1;
	for (std::size_t i = 0; i < incident_half_edges.size(); ++i) {
		reserve_count *= InnerGraph::N_VERTICES_;
	}
	result.reserve(reserve_count);

	std::vector<Int> chosen_targets;
	chosen_targets.reserve(incident_half_edges.size());
	...
	const auto backtrack = [&](auto&& self) -> void {
		if (chosen_targets.size() == incident_half_edges.size()) {
			append_insertion(chosen_targets);
			return;
		}

		for (Int inner_vertex = 0; inner_vertex < InnerGraph::N_VERTICES_; ++inner_vertex) {
			chosen_targets.push_back(
				detail::relabel_inner_vertex_after_insertion<InnerGraph>(
					inner_vertex,
					outer_vertex
...
VectorSpace::LinComb<
	GraInsertLowValentGraph<OuterGraph, InnerGraph>,
	typename OuterGraph::Field
> gra_insert_vertex_unrestricted(
	const OuterGraph& outer,
	Int outer_vertex,
	const InnerGraph& inner,
	typename OuterGraph::Field coefficient = typename OuterGraph::Field{1}
	...
	ResultLinComb result = gra_insert_vertex_unrestricted_raw(
		outer,
		outer_vertex,
		inner,
		coefficient
...
template <typename OuterGraph, typename InnerGraph>
std::unordered_set<GraInsertLowValentGraph<OuterGraph, InnerGraph>>
gra_insertion_support_unrestricted(
	const OuterGraph& outer,
	const InnerGraph& inner
) {
	using ResultGraph = GraInsertLowValentGraph<OuterGraph, InnerGraph>;
	...
	const auto valences = outer.valence_array();
	for (Int vertex = 0; vertex < OuterGraph::N_VERTICES_; ++vertex) {
		if (valences[vertex] == 0) {
			continue;
		}

		auto raw = gra_insert_vertex_unrestricted_raw(
			outer,
			vertex,
			inner,
			typename OuterGraph::Field{1}
		);
		for (const auto& be : raw) {
			typename ResultGraph::Basis canon_basis(be.getValue(), typename OuterGraph::Field{1});
			const auto canon = ResultGraph::canonized(canon_basis);
...
> gra_pre_lie_unrestricted(
	...
	for (Int vertex = 0; vertex < OuterGraph::N_VERTICES_; ++vertex) {
		...
		result += gra_insert_vertex_unrestricted(outer, vertex, inner, coefficient);
	}
...
> gra_lie_unrestricted(
	...
	auto result = gra_pre_lie_unrestricted(left, right, coefficient);
	result = result.add_scaled(gra_pre_lie_unrestricted(right, left, coefficient), Field{-1});
	result.standardize_all();
	result.sort_elements();
	return result;
}
...
template <typename LeftGC, typename RightGC>
GraInsertLowValentGC<LeftGC, RightGC> gra_pre_lie_unrestricted(
	const LeftGC& left,
	const RightGC& right,
	fieldType coefficient = fieldType{1}
) {
	...
	for (const auto& left_be : left.data()) {
		for (const auto& right_be : right.data()) {
			const fieldType pair_coefficient =
				coefficient * left_be.getCoefficient() * right_be.getCoefficient();
			result += gra_pre_lie_unrestricted(left_be.getValue(), right_be.getValue(), pair_coefficient);
		}
	}
...
template <typename LeftGC, typename RightGC>
GraInsertLowValentGC<LeftGC, RightGC> gra_lie_unrestricted(
	const LeftGC& left,
	const RightGC& right,
	fieldType coefficient = fieldType{1}
) {
	...
	ResultLinComb result = gra_pre_lie_unrestricted(left, right, coefficient).data();
	result = result.add_scaled(gra_pre_lie_unrestricted(right, left, coefficient).data(), fieldType{-1});
	result.standardize_all();
	result.sort_elements();
	return ResultGC(std::move(result.raw_elements_nonconst()));
}
